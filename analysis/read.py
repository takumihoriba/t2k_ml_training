...
class WatChMaLOutput(ABC, metaclass=ABCMeta):
    ...
    def plot_training_progression(self, plot_best=True, y_lim=None, fig_size=None, title1=None, title2=None, legend=None):
        """
        ...
        return fig, axs
        """
        fig, (ax1_1,ax2_1) = plt.subplots(2,1)
        ax1_1.set_title(title1)
        ax2_1.set_title(title2)
        ...
        ax1_1.set_ylabel("Loss", c='b')
        ax2_1.set_ylabel("Loss", c='b')
        ax2_1.set_xlabel("Epoch")
        if legend:
            ax1_1.legend(loc=legend)
            ax2_1.legend(loc=legend)
        return fig, ax1_1, ax2_1
    ...
    def read_training_log_from_csv(self, directory):
        ...
        self._log_train = np.array([np.genfromtxt(f, delimiter=',', skip_header=2) for f in train_files])
        self._log_val = np.genfromtxt(directory + "/log_val.csv", delimiter=',', skip_header=2)
        train_iteration = self._log_train[0, :, 0]
        train_epoch = self._log_train[0, :, 1]
        print(train_iteration[train_epoch == 0])
        it_per_epoch = len(train_iteration[train_epoch == 0])
